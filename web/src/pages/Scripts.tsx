...
export default function Scripts() {
  const [form, setForm] = useState({ name: '', description: '', price: '', platform: 'tiktok' });
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold mb-8">Video Scripts</h2>
      ...
      <form onSubmit={handleSubmit} className="bg-[var(--bg-card)] rounded-xl p-6 border border-[var(--border-color)] mb-6">
        ...
        <button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2 rounded-lg bg-[var(--accent-pink)] text-white font-medium disabled:opacity-50">
      ...
      {result && (
        <div className="bg-[var(--bg-card)] rounded-xl p-6 border border-[var(--border-color)]">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-[var(--accent-cyan)]">{result.title}</h3>
