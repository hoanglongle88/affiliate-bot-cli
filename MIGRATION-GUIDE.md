...
> ✅ **Đã tạo xong:** 7 tables (`products`, `video_scripts`, `post_descriptions`, `trend_briefs`, `image_briefs`, `short_video_prompts`, `history`) với indexes và policies
...
| Function                                            | Return type                   |
| --------------------------------------------------- | ----------------------------- |
| `saveShortVideoPrompt(prompt, productId, scriptId)` | `Promise<void>`               |
| `getShortVideoPrompts(limit)`                       | `Promise<ShortVideoPrompt[]>` |
...
| Function                      | Return type                    |
| ----------------------------- | ------------------------------ |
| `listCSVFiles()`              | `{ name, path }[]`             |
| `parseCSVProducts(filePath)`  | `CSVProduct[]`                 |
| `importCSVProducts(products)` | `{ success, skipped, errors }` |
...
### Lỗi "Gemini API đã hết quota (429)"

**Nguyên nhân:** API key Gemini đã hết free tier quota.

**Cách sửa:** Nâng cấp lên paid plan hoặc chuyển sang dùng Ollama (`AI_PROVIDER=ollama`).
